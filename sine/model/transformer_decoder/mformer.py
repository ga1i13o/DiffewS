...
class MFormer(nn.Module):
    ...
    def forward_per_image(
        ...
        pooled_ref_feat_for_ins = self.mask_pooling(ref_feat_for_pooling, sem_mask_for_pooling) # bs, nc, c
    ...
    def forward(
        ...
        for ref_feat, tar_feat, ref_mask, ref_label in zip(ref_features, tar_features, ref_masks, ref_labels):
            out = self.forward_per_image(
                ref_feat[None,], tar_feat[None,], ref_mask[None,], ref_label
            )
            outputs.append(out)
